...
public class phieuChi_Controller implements Initializable {
    @FXML
    private TableColumn<NhaCungCap_Module, String> diaChi_Column;
    ...
    @FXML
    private TableColumn<NhaCungCap_Module, Integer> id_Column;

    @FXML
    private Button luu_Button;

    @FXML
    private TableColumn<NhaCungCap_Module, String> mst_Column;

    @FXML
    private TextField mst_TextField;

    @FXML
    private TableColumn<NhaCungCap_Module, String> nganHang_Column;
    ...
    @FXML
    private TableColumn<NhaCungCap_Module, String> nguoiDD_Column;

    @FXML
    private TextField nguoiDD_TextField;

    @FXML
    private TableColumn<NhaCungCap_Module, Double> noDau_Column;

    @FXML
    private ComboBox<String> noiDung_ComboBox;

    @FXML
    private TableColumn<NhaCungCap_Module, String> sdt_Column;
    ...
    @FXML
    private TableColumn<NhaCungCap_Module, String> ten_Column;
    @FXML
    private TableView<NhaCungCap_Module> table_TableView;

    @FXML
    private Button thoat_Button;

    @FXML
    private TableColumn<NhaCungCap_Module, String> tk_Column;
    ...
    int maPhieu;
    ObservableList<NhaCungCap_Module> data = FXCollections.observableArrayList();
    void setNguoiDD_TextField(){
        FilteredList<NhaCungCap_Module> filteredData = new FilteredList<>(data, b -> true);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        id_Column.setCellValueFactory(new PropertyValueFactory<>("maNCC"));
        ten_Column.setCellValueFactory(new PropertyValueFactory<>("tenNCC"));
        ...
        try {
            preparedStatement = conn.prepareStatement("SELECT noiDungTC FROM danhMucThuChi WHERE loaiTC='Chi'");
        ...
        tao_Button.setOnAction(event -> {
            ...
            try {
                preparedStatement = conn.prepareStatement("SELECT MAX(SOPH+1) FROM PHCHI");
                resultSet = preparedStatement.executeQuery();
                while (resultSet.next()) {
                    maPhieu = resultSet.getInt(1);
                    soPhieu_TextField.setText("PHCHI_" + resultSet.getString(1));
                }
            } catch (Exception e) {
        ...
        luu_Button.setOnAction(event -> {
            try {
                preparedStatement = conn.prepareStatement("phieuCHI_add ?,?,?,?,?");
        ...
        huy_Button.setOnAction(event -> {
            try {
                preparedStatement = conn.prepareStatement("DELETE PHCHI WHERE SOPH = ?");
    ...
    void reload() throws IOException {
        ...
        AnchorPane root = FXMLLoader.load(new File("src/main/resources/com/example/project_i/PhieuChi/phieuChi_Form.fxml").toURI().toURL());
